...
export const ESTATE_EMAIL_NOT_VERIFIED_ERROR_CODE = 404;
export const ESTATE_SERVER_ERROR_CODE = 400;
...
export class CodeError extends Error {
    ...
    static getMessage(category: string, error: string) {
        switch (category) {
            case "Platform":
                switch (error) {
                    case "MaxWorkerLimitReached":
                        return `The total worker limit has been reached. See the ${SERVICE_NOTICES_CHANNEL} channel for when the limit will be increased.`
                    case "UserWorkerLimitReached":
                        return "The maximum number of workers already exist on your account. You must delete an existing worker before you can deploy a new one."
            ...
            case "BusinessLogic":
                switch (error) {
                    case "DuplicateWorkerName":
                        return "A worker already exists with that name."
                    case "WorkerNotFoundByName":
                        return "A worker by that name was not found."
                    case "UserNotFound":
            ...
            case "External":
                return "Estate internal error: " + error;
            case "CodeParse":
                return "Worker compilation error: " + error;
        }
    }
}
