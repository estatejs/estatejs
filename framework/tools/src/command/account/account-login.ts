...
import {logRemoteError, logSuccess, logWarning, logVerbose, logLocalError} from "../../util/logging";
import {AdminKeyFile, getKeyFilePath, isLoggedIn} from "../../model/admin-key-file";
...
export async function loginGoogleIdentity(username: string, password: string) {
...
export async function loginExistingDeveloperAsync(logContext: string, username: string, password: string) {
    await loginGoogleIdentity(username, password);
        ...
        inquirer.prompt([
            {
                type: 'list',
                name: 'login_type',
                message: 'Do you have a Estate account?',
            ...
            if (loginTypeAnswers.login_type === "Yes") {
                ...
                console.log("Please enter your email address. This will be your Estate username.");
...
export function createAccountLoginCommand(before: any) : Command {
    return new Command('login')
        .arguments('[keyFile]')
        .description('Login to Estate optionally specifying your key file')
...
export async function executeLogoutAsync() {
    if (GoogleIdentitySingleton.isLoggedIn) {
        await GoogleIdentitySingleton.logoutAsync();
    }
}
...
export async function createNewAccountAsync(logContext: string, username: string, password: string): Promise<boolean> {
    ...
    if (!createResponse.emailVerified) {
        ...
        while (true) {
            ...
            if (elapsed > 300) {
                console.log(chalk.yellowBright("Timed out while waiting for email verification. Run 'worker account login' to login after you've verified your email address."));
                return false;
            }
    ...
    try {
        ...
        if (loginInfo.isNew) {
            if (await createNewAccountAsync(logContext, loginInfo.username, loginInfo.password)) {
                logSuccess(`Estate account "${loginInfo.username}" created and logged in successfully.`);
            }
        } else {
            await loginExistingDeveloperAsync(logContext, loginInfo.username, loginInfo.password);
            logVerbose(`Successfully logged in with the Estate account "${loginInfo.username}".`);
        }
