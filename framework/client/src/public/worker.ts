import {MessageRegistry, DataRegistry, WorkerRegistry, ServiceRegistry} from "../internal/util/registry.js";
...
class Worker {
    /**
     * (async) Retrieves an existing instance of a Data-derived class from worker process and returns it.
     ...
     * @throws {Error} When a failure occurs when talking to the worker.
     * */
    ...
    /**
     * (async) Pushes one or more Data-derived instances to worker process and saves them to the datastore. Their changes are broadcasted to other clients listening for changes on those objects.
    ...
    async saveDataAsync(...data: Data[]): Promise<void> {
        ...
        const {workerKey, objects} = getDataAndKeyMap(data);
            ...
            for (let message of response.messages) {
                switch (message.valueType()) {
                    case UserMessageUnionProto.DataUpdateMessageProto: {
                        ...
                        handleDataUpdateMessage(logContext, workerKey, update, tracker);
                        break;
                    }
    ...
    /**
     * Closes all open worker connections and removes all Message and Data listeners.
     * This doesn't prevent new connections from being made nor new Message / data listeners from being setup and used.
     * */
    closeConnections() {
        WorkerRegistry.instance.clearRiverClients();
    ...
    /**
     * (async) Subscribes this client to a Message so that when a worker service sends it, this client receives it and calls the listener function.
     ...
     * @return {Promise<void>} A promise that resolves when the worker has subscribed this client to the message.
     * @throws {Error} When a failure occurs when talking to the worker.
     * */
    async subscribeMessageAsync<TS extends Data | Service, TM extends Message>(source: Data | Service, messageType: MessageConstructor<TM>, listener: MessageListener): Promise<void> {
        ...
        const riverClient = await getRiverClientAsync(messageClassKey.workerKey);
     ...
     * @return {Promise<void>} A promise that resolves when the worker has unsubscribed this client from the message.
     * @throws {Error} When a failure occurs when talking to the worker.
     * */
     ...
     */
    removeUpdateListener(data: Data | Data[], listener: DataUpdateListener) : void {
        if (data instanceof Array)
            friendlyArrayArgumentRequired('data', data);
        else
            friendlyArgumentRequired('data', data);
     ...
     * @return {Promise<void>} A promise that resolves when the worker has subscribed this client to object updates made to the objects in question.
     * @throws {Error} When a failure occurs when talking to the worker.
     * */
    async subscribeUpdatesAsync(data: Data | Data[]): Promise<void> {
        ...
        const {workerKey, dataKeys} = getDataAndKeys(data);

        const riverClient = await getRiverClientAsync(workerKey);
        const logContext = createLogContext();
        const response = await riverClient.subscribeDataUpdates(logContext, dataKeys);
     ...
     * @return {Promise<void>} A promise that resolves when the worker has unsubscribed this client from object updates made to the objects in question.
     * @throws {Error} When a failure occurs when talking to the worker.
     * */
    async unsubscribeUpdatesAsync(data: Data | Data[]): Promise<void> {
        ...
        const logContext = createLogContext();
        const response = await riverClient.unsubscribeDataUpdates(logContext, dataKeys);
     ...
     * */
    setOptions(workerOptions: WorkerOptions) {
        options.setWorkerOptions(workerOptions);
    }

    /**
     * Gets the worker's options.
     * */
    getOptions() : WorkerOptions {
        return options.getWorkerOptions();
    }
}
