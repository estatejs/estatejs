...
class DataTransferQueue {
    private _queue: Data[] = [];
    private _workerKey: WorkerKey | undefined;
    private _keys = new KeySet<DataKey>();

    /*Pass in a WorkerKey to make sure all further objects have the same one.*/
    constructor(workerKey?: WorkerKey) {
        this._workerKey = workerKey;
    }

    get workerKey(): WorkerKey | undefined {
    ...
    tryAdd(data: Data): DataKey | undefined {
        ...
        if (this._keys.add(objectKey)) {
            if (this._workerKey) {
                if (!this._workerKey.equals(classKey.workerKey))
                    throw new Error(`Unable to transfer objects from different workers. ${classKey.workerKey.value} is different than ${this._workerKey.value}`);
            } else {
                this._workerKey = classKey.workerKey;
            }
...
function createRequest(b: Builder | number, logContext: string, workerId: Unsigned, workerVersion: Unsigned, kind: UserRequestUnionProto, createInner: CreateInnerFunction): Uint8Array {
    requiresTruthy('logContext', logContext);
    requiresPositiveUnsigned('workerId', workerId);
    requiresPositiveUnsigned('workerVersion', workerVersion);
...
export function createCallServiceMethodRequest(logContext: string, serviceKey: ServiceKey, methodId: MethodId, args: any[]): Uint8Array {
    ...
    const workerKey = serviceKey.classKey.workerKey;

    return createRequest(CALL_SERVICE_METHOD_REQUEST_BUFFER_SIZE, logContext, workerKey.workerId, workerKey.workerVersion, UserRequestUnionProto.CallServiceMethodRequestProto, (builder) => {
        const transferQueue = new DataTransferQueue(workerKey);
...
export function createGetDataRequest(logContext: string, dataKey: DataKey): Uint8Array {
    ...
    const worker_key = dataKey.classKey.workerKey;
    return createRequest(GET_DATA_REQUEST_BUFFER_SIZE, logContext, worker_key.workerId, worker_key.workerVersion, UserRequestUnionProto.GetDataRequestProto, builder => {
...
export function createSaveDataRequest(logContext: string, data: Data[]): Uint8Array {
    ...
    const workerKey = transferQueue.workerKey;
    if (!workerKey)
        throw new Error("Unexpectedly had no worker key");

    return createRequest(builder, logContext, workerKey.workerId, workerKey.workerVersion, UserRequestUnionProto.SaveDataRequestProto, builder => {
...
export function createSubscribeMessageRequest(logContext: string, eventInstanceKey: MessageInstanceKey): Uint8Array {
    ...
    const workerKey = eventInstanceKey.messageClassKey.workerKey;
    return createRequest(CREATE_SUBSCRIBE_MESSAGE_REQUEST_BUFFER_SIZE, logContext, workerKey.workerId, workerKey.workerVersion,
        UserRequestUnionProto.SubscribeMessageRequestProto, builder => {
            const workerRefUnionType = eventInstanceKey.getSourceWorkerReferenceUnionProto();
            if (workerRefUnionType == WorkerReferenceUnionProto.DataReferenceValueProto) {
...
export function createUnsubscribeMessageRequest(logContext: string, eventInstanceKey: MessageInstanceKey): Uint8Array {
    requiresTruthy('eventInstanceKey', eventInstanceKey);
    const workerKey = eventInstanceKey.messageClassKey.workerKey;
    return createRequest(CREATE_UNSUBSCRIBE_MESSAGE_REQUEST_BUFFER_SIZE, logContext, workerKey.workerId, workerKey.workerVersion,
...
export function createSubscribeDataUpdatesRequest(logContext: string, objectKeys: DataKey[]): Uint8Array {
    ...
    let workerKey: WorkerKey | null = null;
    for (const objectKey of objectKeys) {
        if (!workerKey) {
            workerKey = objectKey.classKey.workerKey;
        } else {
            if (!objectKey.classKey.workerKey.equals(workerKey))
                throw new Error('Not all objects are from the same worker');
        }
    ...
    if (!workerKey)
        throw new Error('Unexpectedly workerKey was falsey');

    const referencesVec = SubscribeDataUpdatesRequestProto.createReferencesVector(builder, referencesOffsets);

    return createRequest(builder, logContext, workerKey.workerId, workerKey.workerVersion,
...
export function deserializeValue(logContext: string, workerKey: WorkerKey, value: ValueProto, tracker: Tracker): { type: ValueUnionProto, value: any } {
    requiresTruthy('logContext', logContext);
    ...
    switch (type) {
        ...
        case ValueUnionProto.ObjectValueProto: {
            ...
            for (let i = 0; i < objectValueProto.propertiesLength(); ++i) {
                ...
                const {
                    type: valueType,
                    value
                } = deserializeValue(logContext, workerKey, valueProto, tracker);
        ...
        case ValueUnionProto.DataReferenceValueProto: {
            ...
            if (!primaryKey)
                throw new Error(logError(logContext, 'Unable to deserialize worker object reference because it contained an invalid primary key'));
            return {
                type,
                value: new DataKey(new DataClassKey(workerKey, objectReferenceValueProto.classId()), primaryKey)
            };
        }
        case ValueUnionProto.ServiceReferenceValueProto: {
            ...
            if (!primaryKey)
                throw new Error(logError(logContext, 'Unable to deserialize worker service reference because it contained an invalid primary key'));
            const serviceKey = new ServiceKey(new ServiceClassKey(workerKey, serviceReferenceValueProto.classId()), primaryKey);
            return {
                type,
        ...
        case ValueUnionProto.MapValueProto: {
            ...
            for (let i = 0; i < mapValueProto.itemsLength(); ++i) {
                ...
                const {
                    type: keyType,
                    value: keyValue
                } = deserializeValue(logContext, workerKey, keyValueProto, tracker);

                const {
                    type: valueType,
                    value: valueValue
                } = deserializeValue(logContext, workerKey, valueValueProto, tracker);
        ...
        case ValueUnionProto.SetValueProto: {
            ...
            for (let i = 0; i < setValueProto.itemsLength(); ++i) {
                ...
                const {
                    type: itemType,
                    value: itemValue
                } = deserializeValue(logContext, workerKey, itemValueProto, tracker);
...
function mergeDeltaProperties(logContext: string, target: object, workerKey: WorkerKey, objectKey: DataKey, deltaProto: DataDeltaProto, tracker: Tracker) {
    requiresTruthy('logContext', logContext);
...
export function mergeDelta(logContext: string, workerKey: WorkerKey, deltaProto: DataDeltaProto, tracker: Tracker): void {
    requiresTruthy('logContext', logContext);
    requiresTruthy('workerKey', workerKey);
    requiresTruthy('deltaProto', deltaProto);
    requiresTruthy('tracker', tracker);

    const objectKey = createDataKeyFromDelta(workerKey, deltaProto);
    ...
    if (!deltaObjectVersion || deltaObjectVersion.equals(UnsignedZero))
        throw new Error(logError(logContext, 'Unable to merge worker object because the object version was zero or invalid'));
    ...
    if (deltaProto.deleted()) {
        ...
        mergeDeltaProperties(logContext, object, workerKey, objectKey, deltaProto, tracker);
        ...
        mergeDeltaProperties(logContext, existingObject, workerKey, objectKey, deltaProto, tracker);
        ...
        if (options.getWorkerOptions().enableVerboseLogging) {
...
export function replaceData(logContext: string, workerKey: WorkerKey, objectProto: DataProto | NestedDataProto, tracker: Tracker): Data | null {
    requiresTruthy('logContext', logContext);
    requiresTruthy('workerKey', workerKey);
    requiresTruthy('objectProto', objectProto);
    requiresTruthy('tracker', tracker);

    const classKey = new DataClassKey(workerKey, objectProto.classId());

    const objectVersion = Unsigned.fromLong(objectProto.objectVersion());
    if (objectVersion.equals(UnsignedZero))
        throw new Error(logError(logContext, 'Unable to load worker object because the object version was zero or invalid'));

    const primaryKey = objectProto.primaryKey();
    if (!primaryKey)
        throw new Error(logError(logContext, 'Unable to load worker object because the primary key was invalid'));
...
export function getDataAndKeyMap(data: Data | Data[]):
    { workerKey: WorkerKey, objects: Data[], objectKeyMap: KeyMap<DataKey, Data> } {
    ...
    for (const object of objects) {
        ...
        if (!workerKey) {
            workerKey = classKey.workerKey;
        } else {
            if (!workerKey.equals(classKey.workerKey))
                throw new Error(sysLogError(`Data ${objectKey.getFullyQualifiedName()} doesn't belong to the same worker (${workerKey.getFullyQualifiedName()}) as the others. All objects must be from the same worker.`));
        }
    ...
    if (!workerKey)
        throw new Error(sysLogError('Unexpectedly workerKey was falsey'));

    return {workerKey: workerKey, objects: objects, objectKeyMap: objectKeyMap};
}

export function getDataAndKeys(data: Data | Data[]): { workerKey: WorkerKey, objects: Data[], dataKeys: DataKey[] } {
    requiresTruthy('data', data);
    ...
    return {workerKey: workerKey, objects: objects, dataKeys: objectKeys};
}
...
export function getErrorForNotOkResponse<T extends OkResponseType>(logContext: string, response: UserResponse<T>) {
    if (response.platformException) {
        return new PlatformError(logError(logContext, `Estate encountered an error ${response.platformException.codeString}`), response.platformException.codeString);
...
export function createDataKeyFromDelta(workerKey: WorkerKey, delta: DataDeltaProto): DataKey {
    requiresTruthy('workerKey', workerKey);
    ...
    return new DataKey(new DataClassKey(workerKey, delta.classId()), primaryKey);
}