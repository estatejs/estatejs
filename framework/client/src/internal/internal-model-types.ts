...
import {__Data_Primary_Key} from "../public/symbol.js";

export type WorkerIdString = string;
export type WorkerVersionString = string;
...
export type WorkerKeyString = string;
...
export class WorkerKey implements IKey {
    public readonly value: WorkerKeyString;

    constructor(public readonly workerId: Unsigned, public readonly workerVersion: Unsigned) {
        requiresPositiveUnsigned('workerId', workerId);
        requiresPositiveUnsigned('workerVersion', workerVersion);
        this.value = `${WorkerKey.name}-${this.workerId.toString()}@${this.workerVersion.toString()}`;
    }

    equals(other: WorkerKey): boolean {
        if (!other)
            return false;
    ...
    getName(): string {
        return WorkerRegistry.instance.getName(this);
    }
}
...
export class DataClassKey implements IKey {
    ...
    constructor(public readonly workerKey: WorkerKey, public readonly classId: ClassId) {
        requiresTruthy('workerKey', workerKey);
        requiresTruthy('classId', classId);
        this.value = `${DataClassKey.name}-${workerKey.value}-${classId}`;
    }
    ...
    getFullyQualifiedName(): string {
        return `${this.workerKey.getFullyQualifiedName()}/${this.getName()}`;
    }
...
export class ServiceClassKey implements IKey {
    ...
    constructor(public readonly workerKey: WorkerKey, public readonly classId: ClassId) {
        requiresTruthy('workerKey', workerKey);
        requiresTruthy('classId', classId);
        this.value = `${ServiceClassKey.name}-${workerKey.value}-${classId}`;
    }
...
export class MessageClassKey implements IKey {
    ...
    constructor(public readonly workerKey: WorkerKey, public readonly classId: ClassId) {
        requiresTruthy('workerKey', workerKey);
        requiresTruthy('classId', classId);
        this.value = `${MessageClassKey.name}-${workerKey.value}-${classId}`;
    }
...
export class MessageInstanceKey implements IKey {
    ...
    getSourceWorkerReferenceUnionProto(): WorkerReferenceUnionProto {
        if (this.sourceKey instanceof DataKey)
            return WorkerReferenceUnionProto.DataReferenceValueProto;
        else
            return WorkerReferenceUnionProto.ServiceReferenceValueProto;
    }
}

export class WorkerRegistration {
    constructor(public readonly name: string,
                public readonly workerKey: WorkerKey,
