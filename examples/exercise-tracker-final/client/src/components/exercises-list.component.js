...
import { worker } from 'warp-client';
import { ExerciseAdded, ExerciseTrackerService } from 'MY-WORKER-NAME-worker';
...
export default class ExercisesList extends Component {
    ...
    async componentWillUnmount() {
        ...
        if (this.state.exercises && this.state.exercises.length > 0)
            await worker.unsubscribeUpdatesAsync(this.state.exercises);

        //stop listening for new exercises when we leave this page
        await worker.unsubscribeMessageAsync(this.state.service, ExerciseAdded);
    ...
    async componentDidMount() {
        try {
            ...
            const service = worker.getService(ExerciseTrackerService, 'default');
            this.setState({
                service: service
            });

            //Tell Estate we want to receive the ExerciseAdded message when its sent by the ExerciseTrackerService.
            await worker.subscribeMessageAsync(service, ExerciseAdded, async exerciseAdded => {
                ...
                await worker.subscribeUpdatesAsync(exercise);

                // ... fire onUpdate when it's updated.
                worker.addUpdateListener(exercise, onUpdate);
            ...
            if (exercises && exercises.length > 0) {
                ...
                worker.addUpdateListener(exercises, onUpdate);
            }
        } catch (error) {
            console.error(error);
        }
